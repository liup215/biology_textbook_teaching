...
def split_pdf_by_bookmarks(bookmark_file, pdf_file):
    ...
    for item in root.findall('.//ITEM'):
        ...
        if name and page_str:
            try:
                page = int(page_str)  # PAGE is 1-based
    ...
    for i, bookmark in enumerate(bookmarks):
        try:
            ...
            start_page_index = bookmark['page']

            # Determine end page
            if i + 1 < len(bookmarks):
                # End page is the page before the next bookmark
                end_page_index = bookmarks[i+1]['page'] - 1
            else:
                # If no next bookmark, go to end of file
...
if __name__ == '__main__':
    split_pdf_by_bookmarks('Bookmark_PlantPhysiology.xml', 'PlantPhysiology.pdf')
