...
def split_pdf_by_bookmarks(bookmark_file, pdf_file):
    ...
    for item in root.findall('.//ITEM'):
        ...
        # Filter for main chapters based on indent level and name
        if name and page_str and indent == '1' and name.strip().startswith('Chapter'):
            try:
                # Page numbers in the XML are 1-based, but pypdf uses 0-based indexing.
                page = int(page_str)
                if page <= 0: # Page numbers should be positive
                    print(f"Skipping bookmark with invalid page number (<= 0): {name}")
                    continue
    ...
    for i, bookmark in enumerate(bookmarks):
        try:
            ...
            # For all chapters except the first, set start_page_index to bookmarks[i]['page'] - 1 + 1
            if i > 0:
                start_page_index += 1
...
if __name__ == '__main__':
    split_pdf_by_bookmarks('EconomyOfNatureBookMark.xml', 'EconomyOfNature.pdf')
